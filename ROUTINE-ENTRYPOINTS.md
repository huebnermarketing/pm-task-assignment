...
(2) **Flag** — a signal that needs PM attention but cannot be auto-delegated (PM owns the next move). No Orbit execution by default; the row sits as a PM-readable item. **Also the resting verb for a task due today with no new overnight signal** (`pm_task_due_today`) — surfaced so the PM never has to open Orbit to see today's deadlines (carries a light-read "where this stands" brief, but no auto-assignee). If the PM decides such a due-today Flag needs delegating, a PM note (`delegate to <dev>` / `hand to <pod>`) promotes it to `Create subtask` / `Hand off parent task` in Mode 2 — one of only THREE verb-changing note promotions (the other two: Possible-Orbit-miss → Create parent task, and a `fc_row_type: stale_work_ping` Flag delegated the same way).
...
Fixed-cost projects produce their own Flag-only lane: tracked Active + Fixed-Cost projects (project-wide, not just PM-assigned tasks) generate follow-up-reminder and stale-work-ping Flag rows plus the passive Fixed-Cost Pulse toggle on the dated page — see `collectors/orbit.md` § Fixed-cost extension and `synthesis/fixed-cost-state.md`.
...
**Per-row brief composition is FOUR-source mandatory and default-on, NOT conditional on "the brief looks thin".** For every Create-subtask and Create-parent-task row, before composing the proposed 6-section Orbit body, the matcher MUST read all four input sources in full: (1) the originating Orbit task itself — a single lazy per-row `get_task_details(task_id)` super-call that bundles the full description + the **complete threaded all-time comment history (NOT date-filtered to last_run_timestamp)** + subtasks + attachments, because older comments routinely hold prior decisions / failed attempts / scope changes (`list_task_comments` only as a fallback if the bundled tree looks truncated); (2) every cross-linked Gmail thread end-to-end via the attached `context_signals[]` from Job 4b Pass 1; (3) every Fathom enrichment from Pass 2; (4) any external doc referenced by the above. The workload snapshot + activity_log delta alone are rarely complete enough — the formal task body and the overnight change deltas both routinely miss months of prior conversation. The bias is over-include context, not under-include. See `synthesis/matcher.md` Job 7 § Mandatory deep-read of the originating Orbit task + § Mandatory email-thread enrichment + `schemas/orbit-dq-standard.md` § Source rule for the per-section mapping.
...
Mandatory Orbit collector tool sequence per `collectors/orbit.md` (the collector runs as a **collection sub-agent** for context isolation): TWO non-skippable calls on every run — **(a) `get_user_workload(PM_user_id, is_completed=incomplete, per_page=500)`** is the universe-discovery call (single API call returning every open task assigned to the PM plus the precomputed `workload_summary`, including the deduped `by_project[]` set; replaces per-project iteration of `list_projects` + `get_project_task_list × N`), and **(b) `get_activity_log`** is the change-detection call (every comment, status flip, assignment change, due-date move since last run). `get_activity_log` **requires a `project_id`**, so it is looped once per project in `workload_summary.by_project[]` with `user_id=PM`, `startdate=<last_run YYYY-MM-DD>`, `timezone=Asia/Kolkata`, paged on `has_more`. The Mode 1 sub-step 1f post-collector assertion will abort the run if the workload call is missing or zero projects were queried when the workload had projects (the sub-agent returns these as `workload_fetched` / `activity_log_projects_queried` flags). `get_user_workload` has a second non-collector use — `get_user_workload(candidate_user_id, ...)` is invoked lazily by `synthesis/pod-inference.md` ONLY on the matcher Job 6 no-history fallback path for candidate-availability scoring. See SKILL.md non-negotiable rule #6 for the distinction. The same collection sub-agent also runs fixed-cost discovery (`list_projects` filtered by `type_name="Fixed Cost", status_name="Active", development_owner_id=PM`, plus a mandatory `get_project_details` guard spot-check) and, per tracked project, the fixed-cost event feed per the registry's `Activity source` (shadow: unfiltered activity calls; mail-primary: parsed Orbit-notification mails), plus the roster call per tracked project — per `collectors/orbit.md` § Fixed-cost extension.
...
- Collect overnight signals via fan-out launch: after 1b (lookback determination), launch **1c (Pod Matrix) + 1e (Orbit collection sub-agent: get_user_workload + per-project get_activity_log loop + get_primary_account_manager_users) + 2a (Gmail collector)** all in parallel — they have no inter-dependencies at launch time. 1d (priority pass) is a LOCAL filter + cross-reference computation over 1e's MCP responses (zero new MCP calls of its own); it runs synchronously as soon as 1e responses land. 2a does NOT wait for 1d — Gmail has no Orbit dependency, and the priority-signals-before-matcher invariant is enforced at the matcher Job 1 entry-gate (consumer side), not by serializing collector launches. Window boundary in IST. Fathom is NOT called in collection; it is lazy-fetched by the matcher in Step 3 (Job 4b Pass 2) when a primary signal references a meeting, with a Gmail-attachment fallback (gmail.find_transcript_in_email) before treating Fathom's null as terminal. Orbit collector MUST invoke **`get_user_workload(PM_user_id, is_completed=incomplete, per_page=500)` AND the per-project `get_activity_log` loop (`startdate = last_run_timestamp`, over `workload_summary.by_project[]`)** per the mandatory sequence above — fetched once in the 1e sub-agent and reused by 1d's local filter. The Mode 1 sub-step 1f assertion is the explicit join point for the {1c, 1d, 1e, 2a} fan-out and will abort the run if the workload call is missing or no projects were queried.
- Run Mode 1 sub-step 1f post-collector assertion before synthesis. Abort if the sub-agent's `workload_fetched` is false, or `activity_log_projects_queried` is zero when the workload had ≥ 1 project.
...
- Verify Run Log + Incidents + Fixed-Cost Registry + Preferences are the last four children of the parent, in that order.
- Archive Client-Ask Ledger rows on the Fixed-Cost Registry page that are `Resolved`/`Resolved-manual` with `Resolved on` older than 60 days into the `Archived Asks` toggle (non-blocking on failure — Incident, continue).
