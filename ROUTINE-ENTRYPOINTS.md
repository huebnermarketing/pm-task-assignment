...
(3) **Create parent task** — Gmail-only critical-language signal with NO corroborating Orbit task (Possible Orbit miss). On PM approval, Mode 2 creates a parent Orbit task on the resolved project assigned to the PM, so the PM can spawn sub-tasks under it later. Detection in matcher Job 5 § Possible Orbit miss detection; requires unambiguous project resolution — when project is ambiguous, downgrades to Flag.
...
**Email enrichment is mandatory and default-on, NOT conditional on "Orbit brief looks thin".** For every Create-subtask and Create-parent-task row, the matcher MUST inspect every cross-linked Gmail thread (attached via Job 4b Pass 1 — Gmail collector pulls full thread depth) and every Fathom enrichment, then read through their content (long multi-day threads included) and weave anything load-bearing into the proposed 6-section Orbit body. The Orbit task body alone is rarely complete enough — emails carry AM clarifications, client constraints, scope tweaks, prior decisions, named POCs, and deadline reasoning that never make it into the formal task. The bias is over-include context, not under-include. See `synthesis/matcher.md` Job 7 § Mandatory email-thread enrichment + `schemas/orbit-dq-standard.md` § Source rule for the per-section mapping.
...
- Collect overnight signals: Step 1 (Pull from Orbit) runs 1d Orbit-first priority pass (sequential, blocking — filter the PM-workload response to tasks with due_date=today, then cross-reference get_activity_log for AM-created / AM-reassigned events overnight), then 1e Orbit normal pass + 2a Gmail collector in parallel (Step 2 — Pull from Mail). Window boundary in IST. Fathom is NOT called in collection; it is lazy-fetched by the matcher in Step 3 (Job 4b Pass 2) when a primary signal references a meeting, with a Gmail-attachment fallback (gmail.find_transcript_in_email) before treating Fathom's null as terminal. Orbit collector MUST invoke **`get_user_workload(PM_user_id, is_completed=incomplete, per_page=500)` AND `get_activity_log` with `from_date = last_run_timestamp`** per the mandatory sequence above — both calls are shared across the priority pass and the normal pass (call once, reuse). The Mode 1 sub-step 1f assertion will abort the run if either call is missing.
