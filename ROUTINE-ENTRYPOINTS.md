...
(2) **Flag** — a signal that needs PM attention but cannot be auto-delegated (PM owns the next move). No Orbit execution by default; the row sits as a PM-readable item. **Also the resting verb for a task due today with no new overnight signal** (`pm_task_due_today`) — surfaced so the PM never has to open Orbit to see today's deadlines (carries a light-read "where this stands" brief, but no auto-assignee). If the PM decides such a due-today Flag needs delegating, a PM note (`delegate to <dev>` / `hand to <pod>`) promotes it to `Create subtask` / `Hand off parent task` in Mode 2 — one of only two verb-changing note promotions.
