...
3. **`Fixed-Cost Registry` sub-page** — per `schemas/fixed-cost-registry.md`. If missing, create it with the skeleton: header callout reading `Fixed-cost tracking: 0 active projects` / `Last refresh: <now> · Discovery: filtered` / `Last successful discovery: —` (never) / `Activity source: shadow · Clean audits: 0` / `Managed by PM Task Assignment — edit only the Manual pin rows and the Ask column.` (the full five-line header per `schemas/fixed-cost-registry.md` §1), an empty `Tracked Projects` table, an empty `Client-Ask Ledger` table, and a `Machine state — do not edit` toggle containing an empty `fc_state` block (`{"asks":{},"pings":{}}`). The skeleton's Tracked Projects table starts EMPTY — this forces the collector's fallback logic (`collectors/orbit.md` § Fixed-cost discovery) to run the Appendix-A sweep rather than "reuse" an empty tracked set on a guard trip. **CRITICAL DIFFERENCE from Preferences and the other operational pages:** if `Fixed-Cost Registry` is missing AND cannot be created (e.g., Notion write failure), this is **non-blocking** for Mode 1 — do NOT abort the run. Log an Incident and let the fixed-cost lane skip for this fire (no discovery, no ledger, no reminders/pings); the rest of Mode 1 proceeds normally and the registry is retried on the next fire.
4. **`Preferences` sub-page** — must be the very last child of the parent. If it's not last, move it to the end.

After creation/verification, the bottom four `child_page` blocks on the parent body (in order) must be: `Run Log`, `Incidents`, `Fixed-Cost Registry`, `Preferences`. Year heading-toggle blocks, when present, sit above all four. If `Fixed-Cost Registry` could not be created (non-blocking case above), the bottom three fall back to `Run Log`, `Incidents`, `Preferences` for this fire — `Preferences` still moves to last.

**Hierarchy note (do NOT enforce in preflight):** preflight does NOT create or touch Year/Month heading-toggle blocks on the parent body, does NOT verify dated sub-page placement inside Month toggles, and does NOT auto-relocate flat dated pages found outside the toggle structure. That's `writers/notion.md`'s job at Mode 1 write time, plus drift surfacing during `modes/monthly-archival.md`. Preflight only confirms the operational sub-pages (`Run Log`, `Incidents`, `Fixed-Cost Registry`, `Preferences`) exist and that their `child_page` blocks sit in the correct order at the bottom of the parent body — it does not touch dated content or toggle blocks.
