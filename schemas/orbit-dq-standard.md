...
1. **The originating Orbit task in full** — mandatory per-row deep-read fires during matcher Job 7 (see `collectors/orbit.md` § Per-row deep-read). ONE MCP call per row:
   - `get_task_details(task_id)` — the super-call returning full task body / description / all fields **plus the complete threaded comment tree (all-time, NOT date-filtered to `last_run_timestamp`)** + subtasks + attachments. NOT just the workload snapshot (whose `description` is always null). The collection-phase activity_log returns only the deltas since last_run; older comments (prior client feedback, AM clarifications, decisions, failed attempts, scope changes from earlier weeks/months) are bundled in this call's `comments[]` and routinely hold the load-bearing context for the brief.
   - `list_task_comments(task_id)` — **fallback only**, when the bundled comment tree looks paginated / capped on a very long history.
