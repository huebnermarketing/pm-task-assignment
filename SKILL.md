...
18. **The Morning Queue drives exactly five AI actions, picked per row by `synthesis/matcher.md` Job 5 + Job 5a (work-type classifier) + Job 5.5 (existing-subtask check):**
    ...
    - **`Flag`** — PM-attention signal with no auto-execution. PM owns the next move.
    - **`Create parent task`** — Possible Orbit miss; Gmail-only critical-language signal with NO corroborating Orbit task. On PM approval, Mode 2 creates the parent on the resolved project assigned to the PM.
