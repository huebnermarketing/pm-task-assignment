...
Mode 1 (scheduled collection — 4 conceptual steps, lettered sub-steps below):
  ...
  → Step 1 — Pull from Orbit (1c + 1d + 1e + 2a fan-out launch after 1b; 1f is the join point):
    ...
    → 1e. Orbit normal pass (parallel with 1c/1d/2a — the ONLY Orbit MCP-fetching step in Step 1):
      → collectors/orbit.md normal pass (runs as a collection sub-agent): get_user_workload(PM) + per-project get_activity_log loop (over workload_summary.by_project[]) + get_primary_account_manager_users (AM identity, shared with 1d), plus comments/status/overdue/new tasks not in priority pass
      → Fixed-cost discovery: `list_projects(type_name="Fixed Cost", status_name="Active", development_owner_id=PM)` (filter live, validated 2026-07-06 UAT) unioned with the registry's manual pins → `fixed_cost_projects[]` (collectors/orbit.md § Fixed-cost extension)
      → Filter guard: mandatory plausibility check + `get_project_details` spot-check on `development_owner`; a trip discards the result and falls back to the registry (or an unfiltered sweep) with `discovery_mode: fallback-sweep` + an Incident, never a hard abort (collectors/orbit.md § Fixed-cost extension)
      → Merged activity loop: `workload_summary.by_project[] ∪ fixed_cost_projects[]`, deduped by `project_id` — fixed-cost projects get ONE unfiltered `get_activity_log` call (all actors, no `user_id` filter), a strict superset of the PM-filtered call so overlap costs zero extra calls (collectors/orbit.md § Fixed-cost extension) — conditional on the registry's `activity_source` (shadow / monthly first-Friday re-audit / Gmail-failure fallback only; skipped in mail-primary, where `origin: fixed_cost_mail` signals are the feed — see `collectors/orbit.md` § Activity-source gate)
  ...
  → Step 4 — Synthesize, recommend, write & log:
    ...
    → Job 4c (after Job 4b, before Job 5): synthesis/fixed-cost-state.md — FC-1 resolution scan → FC-2 ask detection → FC-3 follow-up reminders → FC-4 stale-work pings, over the widened Job 4c universe (`origin: fixed_cost_registry` signals + `origin: fixed_cost_mail` signals + Pass-1-cross-linked / Pass-1b-resolved Gmail signals — authoritative definition in `synthesis/matcher.md` Job 4c); mutates the Fixed-Cost Registry's Client-Ask Ledger + `fc_state` block and returns Flag row candidates that join Job 5; registry unavailable → skip silently, non-blocking
    → 4b. synthesis/matcher.md Jobs 5, 5a, 5.5, 6, 7, 7b, 8, 9, 10, 11:
      ...
      → Job 7: compose the 6-section Orbit body (Create subtask + Create parent task + as Reopen-fallback) — MANDATORY per-row deep-read of FOUR sources (unconditional, default-on):
        ① Originating Orbit task in full — per-row `get_task_details` ONLY (the super-call bundles the full description + complete threaded comment tree, all-time, NOT date-filtered + subtasks + attachments — older comments hold prior decisions / failed attempts / scope changes); `list_task_comments` fires only as a fallback when the bundled tree looks truncated. **Issued as parallel batch (cap 25 calls/turn = up to 25 rows, chunked if S > 25 rows); per-row failure → Uncertain AI Note or FAILED-deep-read flag.** `pm_task_due_today` bare Flags join the same batch with a *light* read (`get_task_details` only, newest bundled comments, no fallback) to feed their task_brief — no body/assignee/handoff (matcher Job 7 § Light deep-read for due-today Flags).
    ...
    → 4c. writers/notion.md — write today's dated sub-page with inline database + Ready toggle at top
      ...
      → Renders the Fixed-Cost Pulse toggle (per-project movement counts, passive — no verbs/assignees) and refreshes the Fixed-Cost Registry ledger + `fc_state` block with the fixed-cost tracker's mutations (matcher Job 4c) (writers/notion.md § Flow — refreshing the Fixed-Cost Registry)
    → 4d. Update last-run timestamp in Preferences
    → 4e. writers/run-log.md — append run entry to Run Log database (with Fathom enrichment counts: N attached, M references unresolved, K resolved via gmail-attachment fallback; plus Mode 1 fixed-cost lane stats: discovery mode, projects tracked, signals seen, asks opened/resolved, reminders emitted/suppressed, pings emitted/throttled/dropped, pulse movement count (`pulse_projects_with_movement`))
    → 4f. Exit silently
...
|---|---|---|
| **Orbit** | The PM's open task list — every task where `assignee_id == PM_user_id` returned by a single `get_user_workload(PM)` call. Plus `get_activity_log` for changes since last run (status flips, new comments, assignment changes, due-date moves) — looped once per project in the workload's `by_project[]` set (a `project_id` is required; `user_id=PM` filter, date-bounded to last run). Plus attachments for tasks with new attachment activity. The Mode 1 sub-step 1d priority pass filters the workload to `due_date == today` and cross-references the activity log for AM-created / AM-reassigned events overnight. **Every** task with `due_date == today` (not just the AM-handed ones) also emits a `pm_task_due_today` signal in the normal pass so it surfaces in the queue even with zero overnight activity — the single-pane mandate means the PM never opens Orbit to find today's deadlines (see `collectors/orbit.md` § What to pull, item 6 + matcher Job 5 due-today branch). **User-centric universe — no per-project iteration.** Tasks the PM follows but is not assigned to are out of scope here (they surface via Gmail if relevant). See `collectors/orbit.md` § Universe model. Active + Fixed-Cost projects where `development_owner == PM` are additionally tracked project-wide (activity by ANY actor, not just the PM) via a daily filtered discovery call (`list_projects(type_name="Fixed Cost", status_name="Active", development_owner_id=PM)`) gated by a mandatory filter guard (plausibility check + `get_project_details` spot-check; a trip falls back to the last-known discovery set carried on `registry_snapshot` or an unfiltered sweep), unioned with the manual pin rows carried on `registry_snapshot` — the main routine reads the Fixed-Cost Registry page once after preflight and passes `registry_snapshot` into the collection sub-agent; the collector itself never reads Notion. Those projects get ONE merged, unfiltered `get_activity_log` call per project (a strict superset of the PM-filtered call — zero signal loss, no second sweep) in place of the ordinary PM-scoped loop — but that unfiltered call is **conditional on `activity_source`**: it fires in `shadow` mode, on the monthly re-audit, or on the one-run Gmail-failure fallback only; in `mail-primary` mode otherwise it is skipped and the mail-derived signals (`origin: fixed_cost_mail`) are the event feed instead (`collectors/orbit.md` § Fixed-cost extension). The `get_project_task_list(project_id, is_completed="incomplete")` roster call per tracked project emitting the `open_task_roster[]` that feeds FC-4 stale-work detection stays unconditional in every mode. Tasks the PM follows on OTHER project types remain out of scope per the sentence above — this fixed-cost lane is the sole, narrowly-scoped exception. See `collectors/orbit.md` § Fixed-cost extension. | `mcp__...orbit.*` |
| **Gmail** | PM's `@whitelabeliq.com` inbox ONLY (single account, even if other accounts are authenticated). Aliases respected. Overnight window. Filtered for client/AM/team/leadership. Skips Orbit notification emails, EXCEPT it parses Orbit-notification mails for tracked fixed-cost projects (spec §11.2) — the `mail-primary` event feed for the fixed-cost lane. Each signal emits context-link metadata (project_id_candidates, actor_emails, topic_keywords, timestamp) so matcher Job 4b can corroborate Orbit signals. | `mcp__...gmail.*` |
...
18. **The Morning Queue drives exactly five AI actions, picked per row by `synthesis/matcher.md` Job 5 + Job 5a (work-type classifier) + Job 5.5 (existing-subtask check):**
    ...
    - **`Reopen subtask`** — an existing open subtask of the same `work_type` already sits under the parent; instead of creating a duplicate, the executor flips status, appends a comment with the new work, and reassigns to the last non-PM developer who worked it (extracted from the subtask's bundled `get_task_details(subtask_id).comments`, flattened newest-first). When that last dev is inactive in Orbit, the row drops to `Uncertain:` for PM to pick.
...
24. **Latest-signal anchor required on every row.** Every queue row MUST carry `latest_signal_anchor` — the single most-recent signal across all deep-read sources that triggered the row (source, id, timestamp, author, excerpt, source link). The collector emits a per-task / per-thread `latest_signal` field; the matcher selects the newest across sources as the row's anchor and passes it through; the writer renders it as the first line of the row's H1 Task Brief block (`**Triggered by:** ...`). The render requirement IS the gate — a row with a null anchor cannot render. Structural defense against silent missed-comment failures: collector comment lists are flattened (nested `replies[]` inlined) and sorted newest-first so `comments[0]` is always the trigger by construction — even when the newest comment is a deep reply — and the required-field flow makes "matcher silently skipped the newest comment" impossible without the row visibly failing to render its Triggered-by line.
...
26. **Fixed-cost lane: non-blocking, Flag-only, zero false alarms.** The fixed-cost tracking
    ...
    "any update?" row is forbidden. Discovery never trusts the `development_owner_id` filter
    without the guard (live since 2026-07-06, but a server that loses the param silently
    ignores it and returns the org-wide list with no error — the guard is the regression
    tripwire). Event feed
    per the registry's `Activity source` state machine — Orbit activity log in `shadow`,
    parsed Orbit-notification mails in `mail-primary`, audited weekly via FC-0 (spec §11).
...
- `synthesis/fixed-cost-state.md` — Fixed-cost state tracker (matcher Job 4c): FC-1 resolution scan, FC-2 ask detection, FC-3 follow-up reminders, FC-4 stale-work pings over the widened Job 4c universe — `origin: fixed_cost_registry` + `origin: fixed_cost_mail` + cross-linked/Pass-1b Gmail signals (`synthesis/matcher.md` Job 4c is authoritative).
...
- `schemas/parent-page.md` — parent Notion page structure: header callout + Year/Month heading-toggle blocks on parent body containing dated sub-page links + Run Log + Incidents + Fixed-Cost Registry + Preferences last. Day pages stay as sub-pages (Notion-tree parent = parent page); Year/Month are toggle blocks, not sub-pages.
...
- `schemas/fixed-cost-registry.md` — Fixed-Cost Registry sub-page schema: Tracked Projects table + Client-Ask Ledger + `fc_state` machine-state block. Rendered snapshot, not the source of truth for discovery.
