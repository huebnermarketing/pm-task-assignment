...
- **Job 5.5 — Existing-subtask check.** For `Create subtask` survivors, inspects parent's `subtasks[]` (cached from Job 7's `get_task_details` deep-read). If an open subtask of matching `work_type` exists, flips verb to `Reopen subtask` and sets `last_dev_user_id` from `list_task_comments` (last non-PM commenter). Inactive-dev edge → `Uncertain:` AI Note.
...
- **Job 7 — Generate proposed Orbit body** (Create subtask + Create parent task paths). Full 6-section body per `schemas/orbit-dq-standard.md`, plain language. **Mandatory per-row deep-read of FOUR input sources** before composing the body — all default-on, none gated on whether the workload snapshot or the Orbit task title looks "complete":
  1. **Originating Orbit task in full** — lazy per-row `get_task_details(task_id)` + `list_task_comments(task_id)` (full all-time comment history, NOT date-filtered to `last_run_timestamp` — older comments hold prior decisions / failed attempts / scope changes). See `collectors/orbit.md` § Per-row deep-read.
